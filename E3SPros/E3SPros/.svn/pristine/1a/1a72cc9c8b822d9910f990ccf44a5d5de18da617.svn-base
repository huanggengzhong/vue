...
* description: 短信模板内容
* author: xgzh
* createdDate: 2019年10月21日11:38:48
-->
<template>
  <section class="carTypeConfig">
    <el-col :span="span">
      <lableName :curLabelName="curLabelName" :isShowLabel="isShowLabel" :isRequire="isRequire"></lableName>
      <el-input placeholder="请选择" v-model="modelText" :disabled="disabled" size="small" suffix-icon="el-icon-search" @clear="clearCode" clearable @blur="sendCode" @focus="getFocusCode"></el-input>
      <validateFormate :curLabelName="curLabelName" :validrule="validrule"></validateFormate>
    </el-col>
    <sendMsg :popupsVisible="curPopupsVisible" :codeField="codeField" :textField="textField" :comType="comType" :valueObject="curValueObject" :popupsKey="popupsKey" :code="modelCode" :text="modelText" :key="curPopupsKey" :returnCodeField="returnCodeField" :returnTextField="returnTextField"  :otherField="otherField" :isMul="isMul" @changeCode="getComponentCode"></sendMsg>
  </section>
</template>
<script>
...
import sendMsg from "@/components/crm/crmEjectWindows/crmsendMsg/swd_sendMsg";
...
export default {
  ...
  data() {
    return {
      curLabelName: this.labelName === "" ? "短信模板" : this.labelName
    };
  },
  methods: {
    // 重写显示时执行事件
