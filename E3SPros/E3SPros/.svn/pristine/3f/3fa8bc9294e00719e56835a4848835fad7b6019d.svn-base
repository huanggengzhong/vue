...
export default {
  ...
  data() {
    return {
      ...
      // 同步
      {compKey: 'btnKey2', type: '', size: 'small', clickEvent: () => this.synchrony('cashTypeCode'), text: this.$t('sys.button.sync')},
        // 保存
        {compKey: 'btnKey3', type: '', size: 'small', clickEvent: () => this.save(), text: this.$t('sys.button.save')},
        // 重置
        {compKey: 'btnKey4', type: '', size: 'small', clickEvent: () => this.reset(), text: this.$t('sys.button.reset')},
        // 导出
       {compKey: 'btnKey5', type: '', size: 'small', clickEvent: () => this.exportExcel(), text: this.$t('sys.button.export')}
      ],
      // 动态组件-查询条件
      tableComponents: [
       // 显示组件   品牌名称
      ...
      //  VIN码
       {compKey: 'compKey3', labelName: this.$t('ve.label.vin'), codeField: 'vin', component: () => import('@/components/org/commonInput'), type: 'inputText', isMust: true},
        ...
        {compKey: 'compKey4', labelName: this.$t('org.label.dlrName'), codeField: 'drlId', component: () => import('@/components/org/orgDlr'), type: 'propus', isMust: false},
        // 采购单类型
        {compKey: 'compKey5', labelName: this.$t('ve.label.purchaseOrderTypeCode'), codeField: 'purchaseOrderTypeCode', component: () => import('@/components/ve/PurchaseOrderType'), type: 'dropdownList', isMust: false},
        // 车型
      ...
      tableCols: [
        // 资金类型
        { prop: 'cashTypeName', label: this.$t('ve.label.cashType'), width: 120, align: 'center' },
        // 扣款资金类型
       	    { prop: 'cashTypeCode', label: this.$t('ve.label.deductCashTypeName') + '+', width: 170, align: 'center', isComponent: true,
      ...
      //  车系
       { prop: 'carSeriseName', label: this.$t('org.label.carSerise'), width: 180, align: 'center' },
        { prop: 'carConfigCode', label: '车型编码', width: 0, align: 'center', hidden: true },
      //  颜色价格
    ...
    save() {
      ...
      for(var k in selectData){
        let queryObj = {
          ...
          type: 'mutation',
          // api配置
          apiConfig: veApis.vePurCashChangeUpdate,
          variables: {
