...
export default {
  ...
  data() {
    return {
      // 网格查询API配置对象
      apiConfig: veApis.veAutoConfirmOutLogQry,
      // 动态组件-按钮
      tableButtons: [
        {
          compKey: "btnKey1",
      ...
      tableCols: CacheConfig.cacheData[this.$route.path] && CacheConfig.cacheData[this.$route.path].tableCols && CacheConfig.cacheData[this.$route.path].tableCols.length > 0 ? CacheConfig.cacheData[this.$route.path].tableCols: [
        { prop: 'autoComfirmOutLogId', label: this.$t("ve.label.BattchAssign"), width: 400, align: 'center' },//批次分配日志
        { prop: 'autoComfirmOutLog', label: this.$t("ve.label.BattchAssignLog"), width: 600, align: 'center' }, //批次分配日志内容
        { prop: 'autoComfirmOutDate', label: this.$t("ve.label.BattchAssignLogDate"), width: 300, align: 'center' },//批次分配日志记录时间
        { prop: 'updateControlId', label: this.$t("ve.label.concurrencyControl"),width: 0, align: 'center' ,hidden: true }//并发控制
        ],
        //表单查询数据（查询条件）
      formField:{
        carBrandCode: '',
   ...
   execute(){
      ...
      for(var k in selectData){
        let queryObj = {
          ...
          type: 'mutation',
          // api配置
          apiConfig: veApis.veAutoConfirmOut,
          variables: {
