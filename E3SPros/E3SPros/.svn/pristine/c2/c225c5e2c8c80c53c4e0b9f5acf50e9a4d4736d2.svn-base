...
export default {
                ...
                comps: [
                  {
                    ...
                    clickEvent: this.RemindOrUpdate,
  ...
  methods: {
    ...
    RemindSet() {
      let that = this;
      that.$crmcf.jumpDetailTag(that,"RemindSet","/RemindSet")
    },
    RemindOrUpdate(index){
      let that=this
      let rowData = this.$refs.multipleTable.list[index];
      if(rowData.ruleTypeName=="升级"){
        that.$crmcf.jumpDetailTag(that,"UpdateSet","/UpdateSet",rowData)
      }else{
