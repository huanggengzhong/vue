<template>
  <section>
    ...
    <!-- 短信模板弹窗 -->
    <el-dialog title="协助人" :visible.sync="dialogVisible" @sentData="sendCode" width="900px" :append-to-body="true" center>
...
export default {
  ...
  data() {
    return {
      ...
      input: this.swd_AssistPeople.input,
      dialogVisible: false,
      StaffName: {
        input: ""
      },
  ...
  methods: {
    showdialog: function() {
      let that = this;
      console.log("进来了");
      if (this.swd_AssistPeople.dialogVisible == false) {
        that.dialogVisible = this.swd_AssistPeople.dialogVisible;
      } else {
    ...
    query(val){
      let that = this;
      if(that.$refs.StaffName==undefined&&
    ...
    getData: function() {
      let that = this;
      that.input = this.swd_AssistPeople.input;
      //this.query();
    },
    clear: function() {
      let that = this;
