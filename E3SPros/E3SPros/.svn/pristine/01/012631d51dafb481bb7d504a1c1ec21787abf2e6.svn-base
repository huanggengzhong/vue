<template>
  <el-form-item label="投诉级别">
    <el-select v-model="value" placeholder="请选择" @change="changecode" clearable>
      <el-option
...
export default {
  ...
  data() {
    return {
      value: this.ComplaintLevel.value,
    ...
    getData: function() {
      ...
      let queryObj = {
        ...
        variables: {
          ...
          dataInfo: { lookupTypeCode: "LX105" } //值列表编码
        }
      };
      ...
      that.value=that.ComplaintLevel.value
    },
    reset: function() {
      let that = this;
      this.value = "";
