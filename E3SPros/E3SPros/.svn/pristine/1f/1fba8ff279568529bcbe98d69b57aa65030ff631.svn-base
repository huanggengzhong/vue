<template>
  <section class="opentable">
    <el-dialog
      ...
      :lock-scroll="true"
      :visible.sync="stockSpecialCarVisibled"
      center
      width="1000px"
      @close="close"
    >
      <div
        ...
        style="height: 30px;"
        ref="conTitleHeight"
      >{{$t('ve.label.VehicleInformation1')/*车辆信息*/}}</div>
          ...
          <el-row :gutter="24" class="padding10">
            ...
            <el-col :span="8">
              <el-form-item :label="$t('org.label.carIncolor')/*内饰色*/" prop>
                <el-input
                  v-model="ruleForm.carIncolorCode"
                  :placeholder="$t('sys.tips.esTip0')/*请选择*/"
                  size="small"
                ></el-input>
              </el-form-item>
          ...
          <el-row :gutter="24" class="padding10">
            <el-col :span="8">
              <el-form-item :label="$t('org.label.InventoryLocation')/*库存地点*/" prop>
                <el-input
                  v-model="ruleForm.InventoryLocation"
                  :placeholder="$t('sys.tips.esTip0')/*请选择*/"
                  size="small"
                ></el-input>
              </el-form-item>
      ...
      >
        ...
        <el-table-column
          :label="$t('org.label.sellingPrice')/*销售价*/"
          align="center"
        ...
        <el-table-column
          :label="$t('org.label.costPrice')/*成本价*/"
          align="center"
          current-row-key="carSeriesCode"
        >
          <template slot-scope="scope">{{ scope.row.costPrice }}</template>
        ...
        <el-table-column
          :label="$t('org.label.SpecialPrice')/*特价*/"
          align="center"
          current-row-key="carSeriesCode"
        >
          <template slot-scope="scope">{{ scope.row.specialAmount }}</template>
        </el-table-column>
        <el-table-column
          :label="$t('org.label.SpecialAmount')/*特价金额*/"
          align="center"
          current-row-key="carSeriesCode"
        >
          <template slot-scope="scope">{{ scope.row.specialAmount }}</template>
        ...
        <el-table-column
          :label="$t('org.label.InventoryLocation')/*库存地点*/"
          align="center"
          current-row-key="carSeriesCode"
        >
          <template slot-scope="scope">{{ scope.row.InventoryLocation }}</template>
        </el-table-column>
      </el-table>
      <el-pagination
        background
...
export default {
  ...
  methods: {
    ...
    queryTable(page) {
      ...
      let queryObj = {
        // api配置
        apiConfig: veApis.mdmDlrCarStoreQueryFindAllSearch,
        // 需要调用的API服务配置
        apiServices: [
          {
            //表格中台返回网格的字段
    ...
    savaGrid() {
      ...
      let queryObj = {
        ...
        type: "mutation",
        // api配置
        apiConfig: orgApis.mdmDlrCarStoreMutationSet,
        //条件/实体参数（变量），根据typeParam中的定义配置
        variables: {
          //当前中台使用的名称有dataInfo、info，具体查看API文档
