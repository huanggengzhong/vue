...
export default {
  ...
  data() {
    return {
      carBrandCode:this.editData.carBrandCode,
      // 网格查询API配置对象
      ...
      tableComponents: [
        ...
        },
        {
          ...
          parentFileds: this.carBrandCode,
    ...
    if (this.editData && typeof this.editData === 'object') {
      for (var key in this.formField) {
        if (this.editData[key]) {
          this.formField[key] = this.editData[key]
        } else {
          this.formField[key] = ''
        }
      }
  ...
  methods: {
    // 新增按钮事件
    add() {
      this.editRowData.carConfigId = this.editData.carConfigId
      this.editRowData.carConfigCode = this.editData.carConfigCode
      this.showEdit('add')
    },
    query() {
      this.queryTable1();
