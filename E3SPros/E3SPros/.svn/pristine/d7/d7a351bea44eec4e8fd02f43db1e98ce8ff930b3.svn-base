<template>
  <div>
    ...
    <!-- 弹窗 -->
    <el-dialog title="投诉类别" :visible.sync="dialogVisible" @sentData="sendCode" width="900px" center modal-append-to-body append-to-body>
...
export default {
  ...
  data() {
    return {
      ...
      input: this.swd_complaintCategory.input,
      width: "850px",
      dialogVisible: false,
      cxBig: {
        value: "",
  ...
  methods: {
    ...
    showdialog: function(e) {
      ...
      if (this.swd_complaintCategory.dialogVisible == false) {
        that.dialogVisible = this.swd_complaintCategory.dialogVisible;
      } else {
    ...
    getData: function() {
      let that = this;
      that.input = this.swd_complaintCategory.input;
      this.query();
    },
    query(val) {
      let that = this;
      if(that.$refs.ServiceBigClass==undefined&&
        ...
        that.tabledatas.queryObj.params.serverClassCodeL=that.$refs.ServiceBigClass.value;
        that.tabledatas.queryObj.params.serverClassCodeM=that.$refs.ServiceMidClass.value;
        that.tabledatas.queryObj.params.serverClassCodeS=that.$refs.ServiceSmallClass.value;
      }
    ...
    sendCode(){
      ...
      that.input=that.rowData.bigName+"->"+that.rowData.midName+"->"+that.rowData.smallName
      var cityTemp = [];
      var cityObj = {};
