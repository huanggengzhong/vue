      ...
      <el-row :gutter="26">
        ...
        <el-col :span="22" v-show="toggleParam">
          <el-row>
            ...
            <CarTypeModal :isMul="false" ref="singleCarType" :curPopupsVisible="singleCarTypeVisible" @changeCode="getSingleCarTypeCode" @close="closeSingleCarType" />
            <el-col :span="6">
              <label>状态</label>
...
:doQuery="textQuery" />
<!--选择车型弹窗(多选)-->
<CarTypeModal :isMul="true" ref="multiCarType" :curPopupsVisible="mulCarTypeVisible" @changeCode="getMulCarType" @close="closeMulCarType" />
<!--选择工时弹窗-->
...
export default {
  ...
  data() {
    return {
      ...
      dialogVisible: false,
      //显示隐藏车型弹出框(多选)
      singleCarTypeVisible: false,
      //显示隐藏车型弹出框(多选)
      mulCarTypeVisible: false,
      //显示选择工时弹窗
  ...
  methods: {
    ...
    //查询区获取车型
    getSingleCarTypeCode(val) {
      this.formField.carTypeCode = val[0].carTypeCode;
    },
    //关闭车型单选弹框
    closeSingleCarType(val) {
      val === '!**!' ? this.singleCarTypeVisible = false : ''
    },
    //关闭车型多选弹框
    closeMulCarType(val) {
      val === '!**!' ? this.mulCarTypeVisible = false : ''
    },
     //关闭选择工时弹窗
    closeSeChooseWi(val) {
      console.log(val);
    ...
    modifyData() {
      ...
      } else if (that.rowClickData1.isEnable != '1') {
        this.$alert('该数据已停用，不能修改！', '信息提示', {
          confirmButtonText: '确定',
          type: 'warning',
          callback: action => {
        ...
        // 调用中台API方法（可复用）
         requestGraphQL(paramD).then(response => { 
               ...
               if(response.data[apiSeDbWiQuery.seDbWiMutationRemoveWi.ServiceCode].result === '1') {
                 ...
                 this.$message({
                    type: "warning", 
                    message: response.data.seDbWiMutationRemoveWi.msg
                  });
      ...
      for (var n = 0; n < rows.length; n++) {
        ...
        if (this.queryList2.length != null) {
          for (var m = 0; m < this.queryList2.length; m++) {
            if (this.queryList2[m].carBrandCode == rows[n].carBrandCode && 
            this.queryList2[m].carTypeCode == rows[n].carTypeCode) {
              this.$message({
                type: "warning", 
      ...
      for (var n = 0; n < val.length; n++) {
        ...
        if (this.queryList3.length != null) {
          for (var m = 0; m < this.queryList3.length; m++) {
            if (this.queryList3[m].wiCode == val[n].wiCode) {
              this.$message({
                type: "warning", 
                message: "该工时编码已存在！"  
              });
    ...
    deleteWiRelation() {
      ...
      if(JSON.stringify(that.rowClickData3) == '{}') {
        ...
        if (that.rowClickData3.wiRelationId === '') {
    ...
    uploadSuccess(response) {
        ...
        && response.data.rows.length > 0) {
          ...
          var excelCols = [
            ...
            {name:'工时来源',required:true},
            {name:'经销商编码',required:true},
            {name:'状态',required:true},
            {name:'维修大类',required:true},
            {name:'维修小类',required:true},
            {name:'维修类别',required:true},
            {name:'关键字',required:true},
            {name:'备注',required:true},
          ]
