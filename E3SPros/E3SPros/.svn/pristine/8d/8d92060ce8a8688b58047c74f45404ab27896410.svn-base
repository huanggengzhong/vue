<template>
  <div>
    <el-form-item label="异地网点">
      <el-input
        readonly="readonly"
        v-model="input"
        placeholder="点击选择异地网点"
    ...
    <!-- 弹出框 -->
    <el-dialog :visible.sync="dialogVisible" :append-to-body="true" @sentData="sendCode" width="900px" class="dialog" title="异地网点" center>
...
export default {
  ...
  data() {
    return {
      ...
      input: this.OffsiteNetwork_pop.input,
      width: "1000px",
      dialogVisible: false,
      toggleParam:false,
      cxName: {
  ...
  methods: {
    showdialog: function() {
      let that = this;
      if (this.OffsiteNetwork_pop.dialogVisible == false) {
        that.dialogVisible = this.OffsiteNetwork_pop.dialogVisible;
      } else {
    ...
    query: function() {
      ...
      if (
        ...
        that.tabledatas.queryObj.params.dlrShortName=that.$refs.AgentAbbreviation.input;
        that.tabledatas.queryObj.params.carBrandCode=that.$refs.AgentBrand.value;
        ...
        that.tabledatas.queryObj.params.dlrStatus=that.$refs.AgentStatus.value;
      }
    ...
    getData: function() {
      let that = this;
      that.input = this.OffsiteNetwork_pop.input;
      this.query();
    },
    //行选择事件
