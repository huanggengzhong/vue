<!--
* description: 车型小类下拉框（多选，单选）,支持搜索
* author: luojx
    ...
    <el-col :span="span">
      ...
      >
        <el-option
          ...
          :key="item.smallCarTypeCode"
          :label="item.smallCarTypeCn"
          :value="item.smallCarTypeId"
        >{{item.smallCarTypeCn}}</el-option>
      </el-select>
      <validateFormate
        :curLabelName="curLabelName"
        :validrule="validrule"
...
export default {
  ...
  data() {
    return {
      ...
      // 表格中台返回网格的字段
      apiQueryRow: ["carBrandCode", "smallCarTypeCode","smallCarTypeId", "smallCarTypeCn"],
      listQuery: {
        isEnable: "1",
        carBrandCode: "",
        carSeriesId: ""
