      ...
      <div class="filter-container filter-params-crm">
        <el-form label-position="right" ref="sonForm">
          ...
          <el-row>
            <el-col :span="8">
              <!--升级开始时段-->
              <UpgradeStartTime :UpgradeStartTime="UpgradeStartTime" ref="StartTime"></UpgradeStartTime>
            </el-col>
            <el-col :span="8">
              <!--升级结束时段-->
              <UpgradeStopTime :UpgradeStopTime="UpgradeStopTime" ref="StopTime"></UpgradeStopTime>
            </el-col>
            <el-col :span="8" class="crm-label-required">
              <!--短信模板-->
            ...
            <el-col :span="24" class="effective-clue">
              ...
              <div class="w68">
                ...
                >
                  ...
                  <i
                    :class="{'selectedClass-i-crmFactSysConf':item.checked}"
                  ></i>
                </span>
              </div>
            </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      tabledatas: {
        colnames: [
          { label: "升级阶段", value: "updateStageName" },
          ...
          { label: "执行次数", value: "maxTimesName" },
          { label: "执行模式", value: "execModeName" },
          { label: "周期", value: "inExecPeriod" }
        ],
        tableData: [
          {
            date: "2016-05-01",
        ...
        queryObj: {
          // .后面是服务编码，.前面固定写死
          apiConfig: crmApis.csDbUpdateRemindSetDQueryUpgradeD,
          apiQueryRow: [
            "ruleDId",
            "updateStageName",
            "updateStage",
    ...
    saveSonRule(formName) {
      ...
      let config = {
        ...
        sctMode: [
          ...
          this.$crmcf.crmRegExp("empty"),
          "value",
          that.$refs.sct_ExecutionMode.value,
          "执行模式",
          "不能为空"
        ],
      };
        ...
        if (flag) {
          let variables = {
            ...
            dataInfo: {
              ...
              workDate:"",
              updateStage:that.$refs.UpgradePhase.value,
              maxTimes:that.$refs.ExecuteTimes.value,
              execMode:that.$refs.sct_ExecutionMode.value,
              begTime:that.$refs.StartTime.value,
              endTime:that.$refs.StopTime.value,
              inExecPeriod:that.$refs.ExecuteCycle.input,
              inExecUnit:""
            }
          };
          for (let i = 0; i < that.clueList.length; i++) {
            if (that.clueList[i].checked == true) {
              if (variables.dataInfo.workDate == "") {
                variables.dataInfo.workDate = that.clueList[i].lookupValueCode;
              } else {
                variables.dataInfo.workDate += "," + that.clueList[i].lookupValueCode;
              }
            }
          ...
          that.$crmcf.mutationSumbit(
            that,
            variables,
            crmApis.csDbUpdateRemindSetDMutationUpgradeSave,
            function() {}
          );
        }
      });
