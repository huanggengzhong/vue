<template>
  <div class="app-container app-container-table filter-params-crm-scroll">
    <div class="filter-container filter-button">
      <el-button type="primary" size="small" @click="save('2')">审核</el-button>
        ...
        <el-row>
          <el-col :span="6" class="crm-label-required">
            <el-form-item label="审核说明">
              <el-input v-model="checkdesc" type="textarea" placeholder="请输入" class="long-textarea-style" resize="none"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  methods: {
    ...
    initData(fwdh){
      ...
      requestGraphQL(paramD).then(response => {
        if (
          ...
          that.SSappealId=this.$route.params.appealId
          that.appealdesc=this.$route.params.appealDesc
    ...
    save(val){
      ...
      //校验
      var html=""
      if(that.checkdesc==""||that.checkdesc==null)
      {
        html = html + `<div>请填写审核说明</div>`
      }
      if (html != '') {
        this.$notify.error({
          title: "校验项",
      ...
      //赋值
      var saveObj={}
      that.$set(saveObj, "appealId", that.SSappealId);
      that.$set(saveObj, "appealStatus", val);
      that.$set(saveObj, "dealDesc", that.checkdesc);
      
      let queryObj = {
        // api配置
        type: "mutation",
        apiConfig: crmApis.csBuAppealMutationAudit,
        // 需要调用的API服务配置
        apiServices: [{
            //表格中台返回网格的字段
            apiQueryRow:[]
      ...
      requestGraphQL(paramD).then(response =>{
        if(
          ...
          that.$crmcf.crmcloseView(that,'ComplaintRevPend')
          }else{
            that.$crmcf.showMessages(that,'error',response.data[queryObj.apiConfig.ServiceCode].msg)
          }
      })
