  ...
  <div class="app-container app-container-table">
    ...
    <div class="filter-container filter-params" ref="conHeight">
      <el-row>
        ...
        <el-col :span="6">
          <label>大类编码</label>
          <el-input clearable v-model="listQuery.operatePartCode" placeholder size="small" />
        </el-col>
        <el-col :span="6">
          <label>维修小类</label>
          <el-select
        ...
        <el-col :span="6">
          <label>小类编码</label>
          <el-input clearable v-model="listQuery.repairSmallKindCode" placeholder size="small" />
        </el-col>
      </el-row>
    </div>
...
export default {
  ...
  methods: {
    ...
    loadFaultPartOptions() {
      seDbFaultPartQuery(-1, 1, this.faultPartQuery).then(response => {
        var resData =
          response.data[seApis.seDbFaultPartQueryFindAll.ServiceCode];
        if (resData.result === "1" && resData.rows != null) {
          var listTemp = resData.rows;
          listTemp.forEach(row => {
            this.faultPartOptions.push({
              code: row.faultPartCode,
              text: row.faultPartName
            });
          });
    ...
    queryFindOperateAll(page, firstLoad = "0") {
      ...
      this.bigNode = {};
      //转换了中台请求格式数据
      doQueryFindAll(-1, page || that.pageIndex, that.listQuery).then(
        response => {
          ...
          if (resData.result === "1" && resData.rows != null) {
            ...
            if (resData.rows != null) {
              resData.rows.forEach(row => {
                if (
                  that.list.filter(o => o.operatePartId == row.operatePartId)
                    .length == 0
                ) {
            ...
            if (firstLoad == "1") {
              ...
              resData.rows.forEach(row => {
                if (
                  temp_array.filter(o => o.code == row.operatePartId).length ===
                  0
                ) {
                  temp_array.push({
              ...
              resData.rows.forEach(row => {
                var node = {};
                var nodesExists = treeChilds.filter(
                  o => o.id == row.operatePartId
                );
                ...
                if (row.seDbWiType) {
                  row.seDbWiType.forEach(subrow => {
                    var childExists = node.children.filter(
                      o => o.id == subrow.wiSmallKindId
                    );
    ...
    handleTreeNodeClick(data) {
      const that = this;
      this.operatePartId = data.id;
      // debugger;
      switch (data.deep) {
        case 0:
          that.queryFindOperateAll("1");
          that.bigNode = {};
          break;
        case 1:
          that.queryFindSmall(data.id, "1", function(arr) {
            var tempArr = [];
            arr.forEach(row => {
              tempArr.push({
    ...
    saveBig() {
      ...
      if (
        this.modifyBigModel.faultPartCode == null ||
      ...
      doSaveBig(this.modifyBigModel).then(response => {
        ...
        var resData =
          response.data[seApis.seDbOpratePlaceMutationSaveBig.ServiceCode];
        if (resData.result === "1") {
          that.isSuccess = true;
          that.dialogBigVisible = false;
          this.$message({
            type: "success",
            message: "保存成功"
          });
    ...
    //查询小类
    queryFindSmall(operatePartId, refreshGridOrSelect, callback) {
      //refreshGridOrSelect-1加载网格，0加载联动下拉框
      //加载网格
    ...
    saveSmall() {
      if (
        this.modifySmallViewModel.repairSmallKindCode1 == "" ||
      ...
      this.modifySmallModel.repairSmallKindCode =
        this.modifySmallViewModel.repairSmallKindCode1 +
        this.modifySmallViewModel.repairSmallKindCode2;
      this.listLoading = true;
      this.isSuccess = false;
      ...
      doSaveSmall(this.modifySmallModel).then(response => {
        var resData =
          response.data[seApis.seDbOpratePlaceMutationSaveSmall.ServiceCode];
        that.listLoading = false;
        if (resData.result === "1") {
          that.curTableRow = {};
          that.isSuccess = true;
          that.dialogSmallVisible = false;
          this.$message({
            type: "success",
            message: "保存成功"
          });
          that.queryFindSmall(that.operatePartId, "1");
        } else {
          that.isSuccess = false;
          that.$alert(resData.msg, "提示", {
            confirmButtonText: "确定",
    ...
    addOrModifySmall(oprFlag = "0") {
      ...
      if (oprFlag == "0" || oprFlag == "1") {
        if (oprFlag == "0") {
          ...
          this.modifySmallViewModel.repairSmallKindCode2 = this.curTableRow.repairSmallKindCode.substr(
            this.curTableRow.operatePartCode.length
          );
    ...
    deleteBig() {
      if (
        this.deleteModel.operatePartId == "" ||
        this.deleteModel.updateControlId == ""
      ) {
        this.$message({
          type: "warning",
          message: `请选择要删除的数据行'`
      ...
      doDeleteBig(this.deleteModel).then(response => {
        that.listLoading = false;
        var resData =
          response.data[seApis.seDbOpratePlaceMutationDeleteBig.ServiceCode];
        if (resData.result === "1") {
          that.isSuccess = true;
    ...
    deleteSmall() {
      if (
        this.deleteModel.wiSmallKindId == "" ||
        this.deleteModel.updateControlId == ""
      ) {
        this.$message({
          type: "warning",
      ...
      doDeleteSmall(this.deleteModel).then(response => {
        that.listLoading = false;
        if (
          response.data[seApis.seDbOpratePlaceMutationDeleteSmall.ServiceCode]
            .result === "1"
        ) {
          that.isSuccess = true;
          this.$message({
