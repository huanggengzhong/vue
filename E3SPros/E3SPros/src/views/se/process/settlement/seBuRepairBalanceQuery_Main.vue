<template>
  <div class="app-container app-container-table">
    <div class="filter-container filter-button" ref="searcheHeight">
      <el-button type="primary" size="small" :disabled="isDisabled" @click="saveClick">收款</el-button>
      <el-button size="small" @click="printClick">打印结算单</el-button>
    </div>
    ...
    <el-tabs type="card" v-model="activeName" @tab-click="handleClick" class="filter-container">
      ...
      <el-tab-pane label="维修及备件费用" name="second">
        ...
        >
          ...
          <el-table-column label="工时编码" prop="wiCode" align="center" width="150">
            <template slot-scope="scope">{{ scope.row.wiCode }}</template>
          </el-table-column>
          <el-table-column label="维修内容" prop="wiName" align="center" width="250">
            <template slot-scope="scope">{{ scope.row.wiName }}</template>
          </el-table-column>
          <el-table-column label="付费性质" prop="payKindName" align="center" width="110">
            <template slot-scope="scope">{{ scope.row.payKindName }}</template>
          </el-table-column>
          <el-table-column label="销售工时" prop="saleWorkQty" align="center" width="100">
            <template slot-scope="scope">{{ scope.row.saleWorkQty }}</template>
          </el-table-column>
          <el-table-column label="工时单价" prop="wiPrice" align="center" width="100">
            <template slot-scope="scope">{{ scope.row.wiPrice }}</template>
          </el-table-column>
          <el-table-column label="工时费" prop="wiAmount" align="center" width="100">
            <template slot-scope="scope">{{ scope.row.wiAmount }}</template>
          </el-table-column>
          <el-table-column label="业务类别" prop="businessTypeName" align="center" width="150">
          ...
          <el-table-column label="工时费预估" prop="wiPreAmount" align="center" width="100">
            <template slot-scope="scope">{{ scope.row.wiPreAmount }}</template>
          </el-table-column>
          <el-table-column label="制单责任人" prop="dutyMan" align="center" width="100">
            <template slot-scope="scope">{{ scope.row.dutyMan }}</template>
          </el-table-column>
          <el-table-column label="备注" prop="note" align="center" width="150">
        ...
        <el-table
          class="multipleTable"
          :data="allParam[0].seBuRepairPart"
          element-loading-text="Loading"
          border
          fit
          stripe
        ...
        >
          ...
          <el-table-column label="付费性质" prop="payKindName" align="center" width="150">
            <template slot-scope="scope">{{ scope.row.payKindName }}</template>
          </el-table-column>
          <el-table-column label="数量" prop="partQty" align="center" width="150">
            <template slot-scope="scope">{{ scope.row.partQty }}</template>
          </el-table-column>
          <el-table-column label="单价" prop="partPrice" align="center" width="150">
            <template slot-scope="scope">{{ scope.row.partPrice }}</template>
          </el-table-column>
          <el-table-column label="合计" prop="priceAmount" align="center" width="150">
            <template slot-scope="scope">{{ scope.row.priceAmount }}</template>
          </el-table-column>
          <el-table-column label="业务类别" prop="businessTypeName" align="center" width="150">
            <template slot-scope="scope">{{ scope.row.businessTypeName }}</template>
          </el-table-column>
          <!-- <el-table-column label="维修类别" prop="wiCode" align="center" width="150">
            <template slot-scope="scope">{{ scope.wiCode }}</template>
          </el-table-column>-->
          <el-table-column label="折扣率（%）" prop="partDiscount" align="center" width="150">
            <template slot-scope="scope">{{ scope.row.partDiscount }}</template>
          </el-table-column>
          <el-table-column label="费用预估" prop="prePartAmount" align="center" width="150">
        ...
        <el-table
          class="multipleTable"
          :data="allParam[0].seBuRepairOther"
          element-loading-text="Loading"
          border
          fit
          stripe
        ...
        >
          <el-table-column align="center" label="序号" width="60" type="index" prop="id"></el-table-column>
          <el-table-column label="其他费用类别" prop="otherAmountType" align="center" width="250">
          ...
          <el-table-column label="其他费用" prop="otherAmount" align="center" width="150">
        ...
        <div style="height:470px;overflow-y:auto">
            ...
            <el-row :gutter="26">
              <el-col :span="6">
                ...
                <el-input
                  size="small"
                  :disabled="lockFlag"
                  v-model="paramOperator.repairOrderCode"
                />
              </el-col>
              <el-col :span="6">
                <label>客户名称</label>
                <el-input size="small" :disabled="lockFlag" v-model="paramOperator.custName" />
              </el-col>
              <el-col :span="6">
                <label>车牌号</label>
                <el-input size="small" :disabled="lockFlag" v-model="paramOperator.carLicense" />
              </el-col>
              <el-col :span="6">
                <label>VIN码</label>
                <el-input size="small" :disabled="lockFlag" v-model="paramOperator.vin" />
              </el-col>
              ...
              <el-col :span="6">
                ...
                <el-input
                  ...
                  :disabled="lockFlag"
                  v-model="paramOperator.shouldAmount"
                />
              </el-col>
              <el-col :span="6">
                <label>工时费</label>
          ...
          <el-row :gutter="24">
            ...
            <el-col :span="12">
              ...
              <el-table
                class="multipleTable"
                :data="paramOperator.insertbalanceapplydetail"
                element-loading-text="Loading"
                border
                fit
                stripe
              ...
              >
                <el-table-column align="center" label="序号" width="60" type="index" prop="id"></el-table-column>
                <el-table-column label="付费性质+" align="center" width="150">
                  <!-- <template slot-scope="{row}">
                    <LookupValue
                      ...
                      :isshow="isshow"
                      :code="row.payMode"
                      :looluptype="payKindCode"
                      @changeCode="payKindChange"
                    />
                  </template>-->
                ...
                <el-table-column label="应收单位名称+" align="center" width="120">
                  <template slot-scope="{row}">
                ...
                <el-table-column label="付费方式+" align="center" width="100">
                  <!-- <template slot-scope="{row}">
                    <LookupValue
                      ...
                      :isshow="isshow"
                      :code="row.payType"
                      :looluptype="payTypeCode"
                      @changeCode="payTypeChange"
                    />
                  </template>-->
                  <template slot-scope="scope">
                    <div>
                ...
                <el-table-column label="应收费用+" align="center" width="100">
                  <template slot-scope="{row}">
                    <el-input
                      size="small"
                      oninput="value=value.replace(/[^0-9.]/g,'')"
                ...
                <el-table-column label="发票号+" align="center" width="150">
                  <template slot-scope="{row}">
                    <el-input v-model="row.invoiceNo" size="mini" placeholder="请输入" />
                  </template>
                </el-table-column>
                ...
                <el-table-column fixed="right" label="操作" width="80" align="center">
                  <template slot-scope="scope">
                    <el-button
                      @click.native.prevent="deleteRow(scope.$index, paramOperator.insertbalanceapplydetail)"
                      type="text"
                      size="small"
                    >删除</el-button>
                  </template>
              ...
              >
                ...
                <el-table-column fixed="right" label="操作" width="80" align="center">
                  <template slot-scope="scope">
                    <el-button
                      @click.native.prevent="deleteRow(scope.$index, paramOperator.insertbalancediscount)"
                      type="text"
                      size="small"
                    >删除</el-button>
                  </template>
                ...
                <el-table-column label="折让项目+" align="center" width="100">
                  <template slot-scope="{row}">
                    <LookupValue
                      ...
                      :isMul="isMul"
                      :isshow="isshow"
                      :code="row.discountItemCode"
                      :looluptype="discountItemCode"
                    />
                  </template>
                </el-table-column>
...
export default {
  ...
  created() {
    this.paramOperator.repairOrderCode = this.$route.params.the_data.repairOrderCode;
    this.paramOperator.custName = this.$route.params.the_data.custName;
    this.paramOperator.carLicense = this.$route.params.the_data.carLicense;
    this.paramOperator.vin = this.$route.params.the_data.vin;
    this.paramOperator.shouldAmount = this.$route.params.the_data.shouldAmount;
    this.paramOperator.wiAmount = this.$route.params.the_data.wiAmount;
    this.paramOperator.partAmount = this.$route.params.the_data.partAmount;
    this.paramOperator.otherAmount = this.$route.params.the_data.otherAmount;
  ...
  data() {
    return {
      ...
      payKindList: null, //付费性质值列表
      payTypeList: null, //付费方式值列表
      lockFlag: true,
      ...
      paramOperator: {
        ...
        insertbalanceapplydetail: null,
        //折让
        insertbalancediscount: null
      },
      //结算收款保存参数
      paramSave: {
        oemCode: "",
        ...
        updateControlId: "",
        microPay: null,
        repairBalanceOrder: null,
        balanceOrderDetail: null,
        balanceDiscountItems: null
      },
      num: 0,
      //页面加载参数
      paramLoad: {
        // repairOrderId: this.$route.params.the_data.repairOrderId,
        // repairGatheringId: this.$route.params.the_data.repairGatheringId,
        // repairOrderId: "1",
  ...
  methods: {
    ...
    wiPayTypeChange(index) {
      var payKind = this.paramOperator.insertbalanceapplydetail[index].payMode;
      var payType = this.paramOperator.insertbalanceapplydetail[index].payType;
      if (payKind == "31301") {
        //普通
        if (payType == "3") {
          ...
          this.paramOperator.insertbalanceapplydetail[index].payType = "";
          return;
        }
    ...
    calculate() {
      ...
      var insertbalanceapplydetail = this.paramOperator
        .insertbalanceapplydetail;

      for (var n = 0; n < insertbalanceapplydetail.length; n++) {
        //金额/付费性质-实收金额行
        if (insertbalanceapplydetail[n].payMode == "31301") {
          //1、普通
          this.paramOperator.factCommon =
            this.paramOperator.factCommon +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payMode == "31302") {
          //2、免保
          this.paramOperator.factFree =
            this.paramOperator.factFree +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payMode == "31303") {
          //3、保修
          this.paramOperator.factGuarantee =
            this.paramOperator.factGuarantee +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payMode == "31304") {
          //4、保险
          this.paramOperator.factInsure =
            this.paramOperator.factInsure +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payMode == "31305") {
          //5、内部
          this.paramOperator.factInner =
            this.paramOperator.factInner +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payMode == "31306") {
          //6、协议
          this.paramOperator.factPolicy =
            this.paramOperator.factPolicy +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payMode == "31307") {
          // 7、PDI
        ...
        // 应收费用分担/按付费方式
        if (insertbalanceapplydetail[n].payType == "0") {
          // 1、现金
          this.paramOperator.sumCash =
            this.paramOperator.sumCash +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payType == "1") {
          // 2、银行
          this.paramOperator.sumBank =
            this.paramOperator.sumBank +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payType == "5") {
          // 3、微信
          this.paramOperator.sumWechat =
            this.paramOperator.sumWechat +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payType == "6") {
          // 4、支付宝
          this.paramOperator.sumAlipay =
            this.paramOperator.sumAlipay +
            parseFloat(insertbalanceapplydetail[n].dueAmount);
        } else if (insertbalanceapplydetail[n].payType == "2") {
          // 5、内部
          this.paramOperator.sumInner =
            this.paramOperator.sumInner + insertbalanceapplydetail[n].dueAmount;
        } else if (
          insertbalanceapplydetail[n].payType == "3" &&
          (insertbalanceapplydetail[n].payMode == "31302" ||
            insertbalanceapplydetail[n].payMode == "31303" ||
            insertbalanceapplydetail[n].payMode == "31307")
        ) {
          ...
          insertbalanceapplydetail[n].payType == "3" &&
          insertbalanceapplydetail[n].payMode == "31304"
        ) {
          // 7、保险挂账
          ...
          insertbalanceapplydetail[n].payType == "3" &&
          insertbalanceapplydetail[n].payMode == "31306"
        ) {
          // 8、协议挂账
      ...
      //折让项目遍历
      var insertbalancediscount = this.paramOperator.insertbalancediscount;
      for (var n = 0; n < insertbalancediscount.length; n++) {
        this.paramOperator.sumDiscount =
          this.paramOperator.sumDiscount +
          insertbalancediscount[n].discountAmount;
      }
    },
    ...
    addEmptyDiscount() {
      ...
      var rows = this.paramOperator.insertbalancediscount;
      ...
      //如果没有空白行，则添加空白行
      if (!hasEmpty) {
        this.paramOperator.insertbalancediscount.push({
          repairDiscountId: "",
          repairGatheringId: "",
          balanceOrderCode: "",
          discountItemCode: "",
    ...
    addEmptyShould() {
      //添加空白行（用于新增付费性质）
      var hasEmpty = false; //用于标记是否已经有空白行
      var rows = this.paramOperator.insertbalanceapplydetail;
      for (var n = 0; n < rows.length; n++) {
        if (
      ...
      //如果没有空白行，则添加空白行
      if (!hasEmpty) {
        this.paramOperator.insertbalanceapplydetail.push({
          repairGatheringDId: "",
          repairGatheringId: "",
          balanceOrderCode: "",
          payMode: "",
    ...
    saveClick() {
      // 数据校验
      //1、应收费用分担-按付费性质分配校验
      var rows1 = this.paramOperator.insertbalanceapplydetail;
      ...
      //2、校验折让项目
      var rows2 = this.paramOperator.insertbalancediscount;
      ...
      if (
        ...
        this.paramOperator.shouldAmount
      ) {
      ...
      this.paramSave.balanceOrderDetail = this.paramOperator.insertbalanceapplydetail;
      this.paramSave.balanceDiscountItems = this.paramOperator.insertbalancediscount;
      this.paramSave.repairBalanceOrder = this.allParam.repairBalanceOrder;
      const that = this;
      saveBalance(this.paramSave).then(response => {
        // that.listLoading = false;
        if (
          response.data[apiSeBalance.seBuRepairBalanceSave.ServiceCode]
            .result === "1"
        ) {
          this.isSuccess = true;
          //禁用保存按钮
          ...
          this.errorMsg =
            response.data[apiSeBalance.seBuRepairBalanceSave.ServiceCode].msg;
        }
    ...
    pageLoad() {
      ...
      this.queryType(1);
      // 判断新增还是浏览
      if (this.paramLoad.actionType == "add") {
        // 新增
        this.LoadFees_Add();
      } else if (this.paramLoad.actionType == "view") {
        // 浏览
        this.LoadFees_View();
      }
      this.calculate();
    ...
    LoadFees_Add() {
      ...
      //三、页签三
      //绑定折让项目表格
      this.paramOperator.insertbalancediscount = this.allParam.balanceDiscountItems;
      //绑定应收费用分担-按付费性质分配表
      this.paramOperator.insertbalanceapplydetail = this.allParam.balanceOrderDetail;

      var wiList = this.allParam.repairOrderWIList;
      var partList = this.allParam.repairOrderPartList;
      var otherList = this.allParam.repairOrderOtherFeeList;

      //1、计算维修应收费用：工时、备件、其他
      if (wiList != null && wiList.length > 0) {
        for (var n = 0; n < wiList.length; n++) {
          this.paramOperator.wiAmount =
            this.paramOperator.wiAmount + wiList[n].wiPreAmount;
        }
      }
      if (partList != null && partList.length > 0) {
        for (var n = 0; n < partList.length; n++) {
          this.paramOperator.partAmount =
            this.paramOperator.partAmount + partList[n].prePartAmount;
        }
      }
    ...
    queryLoadDetail(page) {
      //获取父页面参数
      // this.paramLoad.repairGatheringId = this.$route.params.the_data.repairGatheringId;
      // this.paramLoad.repairOrderCode = this.$route.params.the_data.repairOrderCode;
      // this.paramLoad.repairOrderId = this.$route.params.the_data.repairOrderCode;
      // this.paramLoad.vin = this.$route.params.the_data.vin;
      // this.paramLoad.oemCode = this.$route.params.oemCode;
      // this.paramLoad.groupCode = this.$route.params.groupcode;
      // this.paramLoad.actionType = this.$route.params.actionType;

      const that = this;
      doInit(10, page, that.paramLoad).then(response => {
        if (
          response.data.fiBuRepairGatheringQueryFindAllReceivablesRD.result ===
          "1"
        ) {
          that.allParam =
            response.data[
          ...
          //判断是否已经收款 31101:未结算
          if (this.allParam.repairBalanceOrder.gatheringState == "31101") {
            this.paramLoad.actionType = "add";
          } else {
            this.paramLoad.actionType = "view";
          }
          //页面加载
          this.pageLoad();
        }
