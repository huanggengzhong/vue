...
export default {
  ...
  methods:{
    ...
    handleRowClick(row,event,column){  //行点击事件
      ...
      this.flag = column.isTrusted;  // 点击后flag变成true
    },
    ...
    closeeditDialogMaintenance(val){  //弹窗关闭
        ...
        this.editDialogMaintenanceVisible = false;
                             //  [1]:true取消1  [2]:false保存0
       // this.flag = val.split(" ")[1] === 'true' ? true : false;
        if(val.split(" ")[2] == a){
          this.flag = false;
          this.fetchData(1);
          console.log("0"+this.flag);
        }else{
          this.flag = true;
    ...
    closeDialogMaintenance(val){
      this.editDialogMaintenanceVisible = val;
    },
