<template>
  <div class="app-container app-container-table">
      ...
      <el-row :gutter="24">
        <el-col :span="22" class="table-col">
          <LookupValue
            ...
            :lookuptype="lookuptype[0].type"
            :labelName="lookupValuelable[0].lable"
            @changeCode="getLookupValue1"
          />
          <!-- <seComQueChoDropDownList :span="6" :labelName="labelName" @changeCode="getComQueChoDrop" :code="listQuery.dlrShortName" /> -->
    ...
    >
      ...
      <el-table-column label="单据类型" align="center" prop="billTypeName">
        <template slot-scope="scope">{{ scope.row.billTypeName }}</template>
      </el-table-column>
      <el-table-column label="单据编号" align="center" width="160" prop="outStoreCode">
      ...
      <el-table-column label="领料人" align="center" prop="pickingPersonName">
        <template slot-scope="scope">{{ scope.row.pickingPersonName }}</template>
      </el-table-column>
      <el-table-column label="VIN" align="center" width="170" prop="vin">
      ...
      <el-table-column label="数量" align="center" prop="outStoreQty">
      ...
      <el-table-column label="销售总价" align="center" prop="outStoreQty">
        <template slot-scope="scope">{{ scope.row.outStoreQty }}</template>
      </el-table-column>
      ...
      <el-table-column label="出库原因" align="center" prop="remark">
      ...
      <div style="height:465px;overflow-y:auto;width:100%">
        <div ref="print">
          ...
          <el-row class="textCenter" v-for="(item, index) in list" :key="index">
            ...
            <el-col :span="2" aglin="center" class="border1 height line-height">{{item.partProfit}}</el-col>
          </el-row>
  ...
  methods: {
    ...
    fetchData(page,dataType) {
      ...
      paBuDlrOutStoreDQueryFindAll(
        ...
        if(dataType == 'excel'){
          this.$utils.downloadFile(response, '备件出库单查询明细.xlsx')
          this.listLoading = false
        } else if (
          response.data[paApis.paBuDlrOutStoreDQueryFindAll.ServiceCode].result === "1") {
          this.list = response.data[paApis.paBuDlrOutStoreDQueryFindAll.ServiceCode].rows;
          this.pageTotal = response.data[paApis.paBuDlrOutStoreDQueryFindAll.ServiceCode].records;
          this.listLoading = false;
        } else {
          this.listLoading = false;
        }
