...
export default {
  ...
  data() {
    return {
      // 网格查询API配置对象
      apiConfig: veApis.vetransferDetailQueryForPage,
     ...
     tableComponents: CacheConfig.cacheData[this.$route.path] && CacheConfig.cacheData[this.$route.path].tableComponents.length > 0 ? CacheConfig.cacheData[this.$route.path].tableComponents : [
        ...
        {compKey: 'compKey8', labelName: this.$t('ve.label.transferApplyStaus'), /*调拨申请状态*/ lookuptype: 'VE0488', codeField: 'transferApplyStaus', component: () => import('@/components/org/LookupValue'), type: 'dropdownList', isMust: false},
        {compKey: 'compKey9', labelName: this.$t('ve.label.transferConfirmStartDate'),/*调拨确认开始日期*/ codeField: 'transferConfirmStartDate', component: () => import('@/components/org/datePicker/datePicker'), type: 'datePicker', dateOptionsType: '0', isMust: false },
        {compKey: 'compKey10', labelName: this.$t('ve.label.transferConfirmEndDate'),/*调拨确认结束日期*/ codeField: 'transferConfirmEndDate', component: () => import('@/components/org/datePicker/datePicker'), type: 'datePicker', dateOptionsType: '0', isMust: false },
      ],
      // 动态生成网格列
      tableCols: CacheConfig.cacheData[this.$route.path] && CacheConfig.cacheData[this.$route.path].tableCols.length > 0 ? CacheConfig.cacheData[this.$route.path].tableCols : [
        ...
        { prop: 'transferType', label: this.$t('ve.label.transferType')/*调拨类型*/, width: 200, align: 'center' },
        ...
        { prop: 'transferApplyDate', label: this.$t('ve.label.transferApplyDate')/*调拨申请日期*/, width: 150, align: 'center' },
        { prop: 'computerExamineDate', label: this.$t('ve.label.computerExamineDate')/*电脑审核日期*/, width: 150, align: 'center' },
        { prop: 'transferOutExamineDate', label: this.$t('ve.label.transferOutExamineDate')/*调出审核日期*/, width: 150, align: 'center' },
        { prop: 'transferInConfirmDate', label: this.$t('ve.label.transferInConfirmDate')/*调入确认日期*/, width: 150, align: 'center' },
        { prop: 'PVExamineDate', label: this.$t('ve.label.PVExamineDate')/*PV审核日期*/, width: 150, align: 'center' },
        { prop: 'updateControlId', label: this.$t('ve.label.updateControlId')/*并发字段*/, hidden: true, align: 'center' }
      ],
       //表单查询数据
      formField: {
        carBrandCode:"",
        ...
        carIncolorId:"",
        transferApplyStaus:"",
        transferConfirmStartDate:"",
        transferConfirmEndDate:""
      },
      };
  },
   methods: {
