  ...
  data() {
    return {
      ...
      tableButtons: [
        ...
        {compKey: 'btnKey2', type: '', size: 'small', clickEvent: () => this.save(), text:this.$t('sys.button.check')},// 审核
  ...
  methods: {
    getCols() {
      ...
      if (CacheConfig.cacheData[this.$route.path] && CacheConfig.cacheData[this.$route.path].tableCols.length > 0) {
        ...
        cols = cols.concat([
        //单据状态
        { prop: "returnStatusName", label: this.$t('ve.label.receiptState'),  align: "center" },
        //发票状态
        { prop: "invoiceApplyStatusName", label: this.$t('ve.label.invoiceApplyStatusName'),  align: "center" },
         // 发票处理方式+
        {
        prop: "invoiceHandleMethod",
        label: this.$t('ve.label.invoiceHandleMethod'),//this.$t('org.label.targetValue')+'+'
        width: 120,
        align: "center",
        isComponent: true,
        comps: [
        ...
        for(var i in selectData){
            let queryObj = {
				...
				variables: {
					...
					dataInfo: {
                         ...
                         "purReturnDId": selectData[i].purReturnDId,
                         "returnDlrId": selectData[i].returnDlrId,
                         "purOrderDId": selectData[i].purOrderDId,
                         "invoiceState": selectData[i].invoiceState,
                         "returnStatus": selectData[i].returnStatus,
                         "updateReturnStatus": selectData[i].updateReturnStatus,
                         "updateControlIdCarStore": selectData[i].updateControlIdCarStore,
                         "updateControlIdVeCarStore": selectData[i].updateControlIdVeCarStore,
         ...
         //mock数据先不判断
				// if (response.data[queryObj.apiConfig.ServiceCode].result === '1') {
                    // if(i === length){
                        that.$message({
                            message: this.$t('sys.tips.esTip12')/*保存成功*/,
                            type: 'success',
                            duration: 2000,
                        });
                        this.queryTable(1)
