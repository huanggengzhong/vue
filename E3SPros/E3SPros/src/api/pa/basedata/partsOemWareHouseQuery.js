...
export function doQueryList(pageSize, pageIndex, dataInfo) {
  ...
  const queryObj = {
    ...
    typeParam:
      ...
      paApis.partsOemWareHouseQueryFindAll.InputType +
      ')',
    // 请求API
    apiUrl: paApis.partsOemWareHouseQueryFindAll.APIUrl,
    // 需要调用的API服务配置
    apiServices: [
      {
        // API服务编码&参数
        apiServiceCode: paApis.partsOemWareHouseQueryFindAll.ServiceCode,
        apiServiceParam:
...
export function submitForm(dataInfo) {
  // 保存关联接口
  const queryObj = {
    // 请求类型&参数 保存mutation  查询query
    type: 'mutation',
    typeParam:
      '($dataInfo: ' + paApis.partsOemWareHouseMutationSave.InputType + ')',
    // 请求API
    apiUrl: paApis.partsOemWareHouseMutationSave.APIUrl,
    // 需要调用的API服务配置
    apiServices: [
      {
        // API服务编码&参数
        apiServiceCode: paApis.partsOemWareHouseMutationSave.ServiceCode,
        apiServiceParam: '(dataInfo: $dataInfo)',
        // 表格中台返回网格的字段
        apiQueryRow: []
      }
