        ...
        <el-col :span="type === 0 ? 24: 16">
          <div class="filter-container filter-params">
            <el-row>
              <el-col :span="8">
                <label>备件编码</label>
                <el-input v-model="listQuery.dataInfo.partNo" size="small" />
              </el-col>
              <el-col :span="8">
                <label>备件名称</label>
                <el-input v-model="listQuery.dataInfo.partName" size="small" />
              </el-col>
              <el-col :span="8">
                <label>备件类别</label>
                <el-select
                ...
                >
                  <el-option
                    v-for="(item, i) in paType"
                    :key="`paType_${item.partTypeId}${i}`"
              ...
              <el-col :span="8">
                ...
                >
                  <el-option
                    v-for="(item, i) in partProperty"
                    :key="`partProperty${item.lookupValueCode}${i}`"
              ...
              <el-col :span="8">
                ...
                >
                  <el-option
                    v-for="(item, i) in pubSeries"
                    :key="`pubSeries_${item.carSeriesCode}${i}`"
              ...
              <el-col :span="8">
                ...
                >
                  <el-option
                    v-for="(item, i) in pubCar"
                    :key="`pubCar_${item.carTypeCode}${i}`"
                    :label="item.label"
              ...
              <el-col :span="8">
                <el-checkbox v-model="listQuery.dataInfo.isRefine" size="small">精品</el-checkbox>
              </el-col>
              <el-col :span="8">
                <el-checkbox v-model="listQuery.dataInfo.selectParts" size="small">显示已选备件</el-checkbox>
              </el-col>
              <el-col :span="8">
                <el-checkbox v-model="listQuery.dataInfo.notShowZero" size="small">显示库存数量为0的备件</el-checkbox>
              </el-col>
              <el-col :span="24">
                <div class="filter-container filter-button">
                  <el-button type="text" size="small" @click="downloadExcel()">导入摸板下载</el-button>
                  ...
                  <el-button size="small" @click="replaCementChoose">替换件选择</el-button>
          ...
          >
            ...
            <el-table-column label="备件车系" width="80" align="center">
              <template slot-scope="scope">{{ scope.row.pubSeriesName }}</template>
            </el-table-column>
            <el-table-column label="备件车型" width="80" align="center">
              <template slot-scope="scope">{{ scope.row.pubCarTypeCode }}</template>
            </el-table-column>
            ...
            <el-table-column label="可用库存" width="80" align="center">
              <template slot-scope="scope">{{ scope.row.canUseQty }}</template>
            </el-table-column>
...
//   watch: {
//     repairOrderModels(newValue, oldValue) {
//       if (newValue) {
//         this.dataInfo.ownerType = newValue.DLR_ID;
//       }
//     }
//   },
  data() {
  ...
  created() {
    this.type === 1
      ? (this.thimeTitle = "备件多选")
      : (this.thimeTitle = "备件单选");
    // this.initFetchData()
  ...
  methods: {
    //替换件选择
    replaCementChoose() {
      if (this.rowData.partNo != "" && this.rowData.partNo != undefined) {
        this.partNo = this.rowData.partNo;
        this.sparePartInfo.partNo = this.partNo;
        this.sparePartInfo.dlrId = this.$store.getters.orgInfo.DLR_ID;
    ...
    uploadSuccess(response) {
      if (
        ...
        var implistTemp = [];
        //导入Excel模块列头、数据是否必填
        var excelCols = [{ name: "备件编号", required: true }];
        ...
        for (var i = 0; i < list.length - 1; i++) {
          this.importQuery.partNo =
            this.importQuery.partNo + list[i]["备件编号"] + ",";
        }
        this.importQuery.partNo =
          this.importQuery.partNo + list[list.length - 1]["备件编号"];
      ...
      let queryObjA = {
        ...
        apiServices: [
          ...
          },
          {
            ...
            apiQueryRow: ["partTypeId", "partTypeName"]
          }
        ],
    ...
    fetchData() {
      ...
      if (this.dataInfo.isRefine == true) {
        this.dataInfo.isRefine = "1";
      }
      if (this.dataInfo.selectParts == true) {
        this.dataInfo.selectParts = "1";
      }
      if (this.dataInfo.notShowZero == true) {
        this.dataInfo.notShowZero = "1";
      }
      var partNos = this.dataInfo.partNo.split(",");
      if (partNos.length > 1) {
        this.dataInfo.isMuilty = "1";
        this.dataInfo.partNo = this.dataInfo.partNo;
      }
      this.dataInfo.dlrId = this.$store.getters.orgInfo.DLR_ID;
         this.dataInfo.ownerType = this.repairOrderModels.dlrId;
        debugger
      doPaChoosePartQuery(
        this.listQuery.pageSize,
        this.listQuery.pageIndex,
    ...
    getPubCarTypeCode(val) {
      ...
      requestGraphQL(paramD).then(response => {
        if (
          response.data[seApis.seChooseCarTypeQueryFind.ServiceCode].result ===
          "1"
        ) {
          this.pubCar = "";
          this.pubCar =
            response.data[seApis.seChooseCarTypeQueryFind.ServiceCode].rows;
