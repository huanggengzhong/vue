    ...
    >
        ...
        >
          <div class="filter-container filter-button">
            ...
            <el-button
              ...
              @click="sendCode"
            >确认</el-button>
            <el-button size="small" @click="curPopupsVisible=false">取消</el-button>
          </div>
        </el-col>
      </el-row>
      ...
      <div class="filter-container filter-params">
        <el-row style="margin-bottom:0">
          ...
          ></lableName>
          <el-input size="small"  v-model="secvalue" placeholder="请输入城市名称"></el-input>
          </el-col>
        </el-row>
      </div>
        ...
        apiServices: [
          {
            // 表格中台返回网格的字段
            apiQueryRow: [
              "mdmOrgProvinceExt.provinceName",
              "mdmOrgProvinceExt.provinceCode",
              "provinceId",
              "cityId",
              "cityName",
            ]
        ...
        if (
          ...
          for (var i = 1; i < this.distributorsInfo.length; i++) {
            ...
            for (var j = 0; j < distributorsInfoTemp.length; j++) {
              if (distributorsInfoTemp[j].provinceName == this.distributorsInfo[i].mdmOrgProvinceExt.provinceName) {
                  flag = false;
                  break;
              }
            }
            if (flag) {
              provinceObj["provinceName"] = this.distributorsInfo[i].mdmOrgProvinceExt.provinceName;
              provinceObj["selected"] = false;
              provinceObj["indeterminate"] = false;
              provinceObj["id"] = this.distributorsInfo[i].provinceId;
              provinceObj["provinceCode"] = this.distributorsInfo[i].mdmOrgProvinceExt.provinceCode;
          ...
          for (var i = 0; i < distributorsInfoTemp.length; i++) {
            var cityList = this.distributorsInfo.filter(o => o.mdmOrgProvinceExt.provinceName == distributorsInfoTemp[i].provinceName);
