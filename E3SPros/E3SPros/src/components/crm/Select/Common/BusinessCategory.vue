<template>
    <el-form-item label="业务类别" :prop="checkprop">
        <el-select v-model="value" @change="sendCode" placeholder="请选择" clearable>
            <el-option v-for="item in options" :key="item.lookupValueCode" :label="item.lookupValueName"
...
export default {
    ...
    data() {
        return {
            value: this.BusinessCategory.value,
            checkprop:this.BusinessCategory.checkprop,
    ...
    methods: {
        getData: function() {
            ...
            this.value = this.BusinessCategory.value;
        },
        reset: function() {
            let that = this;
            this.value = "";
		...
		getNewData() {
            let that = this;
            that.value = this.BusinessCategory.value;
		},
