<template>
  <el-form-item label="线索自动失效" prop="clueAutoInvalid.value" clearable>
...
export default {
  ...
  data() {
    return {
      value: this.ClueAutoFailure.value,
      options:''
    };
  },
  mounted: function() {
    ...
    getData: function() {
      ...
      let queryObj = {
        ...
        variables: {
          ...
          dataInfo: {lookupTypeCode:'LX809'}//值列表编码
        }
      };
      ...
      that.value=this.ClueAutoFailure.value
    },
    reset:function(){
      let that=this
      this.value=''
