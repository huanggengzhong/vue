<template>
  <div>
      <el-form-item label="试乘试驾车型" prop="inteTypeCode.input">
        <el-input
          readonly="readonly"
          v-model="input"
          placeholder="点击选择车型"
    ...
    <el-dialog title="试乘试驾车型" :visible.sync="dialogVisible" :append-to-body="true" @sentData="sendCode" width="900px" center>
...
export default {
  ...
  data() {
    return {
      ...
      input: this.TestDrvModel.input,
      pcode: this.TestDrvModel.pcode,
      width: "800px",
      dialogVisible: false,
      cxConfig: "",
      tabledatas: {
  ...
  methods: {
    showdialog: function() {
      let that = this;
      if (this.TestDrvModel.dialogVisible == false) {
        that.dialogVisible = this.TestDrvModel.dialogVisible;
      } else {
    ...
    query: function() {
      let that = this;
      that.tabledatas.queryObj.params.carSeriesId=that.TestDrvModel.pcode
    ...
    getData: function() {
      let that = this;
      that.input = this.TestDrvModel.input;
      that.pcode = this.TestDrvModel.pcode;
      this.query();
    },
    //行选择事件
